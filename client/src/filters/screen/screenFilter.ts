...
export class ScreenFilter extends Filter {
    ...
    override apply(filterManager: FilterSystem, input: RenderTexture, output: RenderTexture, clear: CLEAR_MODES): void {
        ...
        this.uniforms.uRange = SubControl.current?.submarine?.stats.lightEffectiveRange() ?? 0;
        this.uniforms.uSonarRange = SubControl.current?.submarine?.stats.sonarEffectiveRange() ?? 0;
